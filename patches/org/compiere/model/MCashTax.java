 ...
 */
public class MCashTax extends X_C_CashTax
{
	/**
	 * 
	 */
	...
	/** Cash Taxes */
	public static MCashTax[] m_taxes;
	 ...
	 */
	public static MCashTax get (MCashLine line, int precision, 
		boolean oldTax, String trx)
	{
		MCashTax retValue = null;
		...
		try
		{
			...
			if (rs.next ())
			{
				retValue = new MCashTax (line.getCtx(), rs, trx);
			}
			rs.close ();
			pstmt.close ();
			pstmt = null;
		...
		//	Create New
		retValue = new MCashTax(line.getCtx(), 0, trx);
		retValue.set_TrxName(trx);
	...
	/**	Static Logger	*/
	private static CLogger	s_log	= CLogger.getCLogger (MCashTax.class);
	 ...
	 */
	public MCashTax (Properties ctx, int ignored, String trx)
	{
		super(ctx, 0, trx);
		if (ignored != 0)
	 ...
	 *	@param trx transaction
	 */
	public MCashTax (Properties ctx, ResultSet rs, String trx)
	{
		super(ctx, rs, trx);
	}	//	MInvoiceTax
	...
	{
		StringBuffer sb = new StringBuffer ("MCashTax[");
	 ...
	 */
	public static MCashTax[] getTaxes(boolean requery,MCash cash) {
		if (m_taxes != null && !requery)
			return m_taxes;
		String sql = "SELECT * FROM C_CashTax WHERE C_Cash_ID=?";
		ArrayList<MCashTax> list = new ArrayList<MCashTax>();
		PreparedStatement pstmt = null;
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			while (rs.next())
				list.add(new MCashTax(cash.getCtx(), rs, cash.get_TrxName()));
			rs.close();
			pstmt.close();
			pstmt = null;
		} catch (Exception e) {
		...
		m_taxes = new MCashTax[list.size()];
	...
	public static boolean calculateTaxTotal(MCash cash) {
		...
		for (MCashLine line : lines) {
			...
			if (!taxList.contains(taxID)) {
				MCashTax iTax = MCashTax.get(line, getPrecision(), false,
		...
		MCashTax[] taxes = getTaxes(true,cash);
		for (MCashTax iTax : taxes) {
			...
			if (tax.isSummary()) {
				...
				for (MTax cTax : cTaxes) {
					...
					//
					MCashTax newITax = new MCashTax(cash.getCtx(), 0,
							cash.get_TrxName());
