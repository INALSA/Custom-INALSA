...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	public boolean reverseCorrectIt()
	{
		...
		/*
		 * Modificado por: Yamel Senih 07/04/2011 11:37
		 * Agregar un tipo de documento distinto al reversar la factura
		 */
		
		int tipoDocReverso = ((MDocType) getC_DocTypeTarget()).get_ValueAsInt("Void_C_DocType_ID");
				
		
		if(tipoDocReverso == 0){
			tipoDocReverso = getC_DocType_ID();
		}
		
		//	Deep Copy
		MInvoice reversal = copyFrom (this, getDateInvoiced(), getDateAcct(),
				tipoDocReverso, isSOTrx(), false, get_TrxName(), true);
		
		/*
		 * Fin de Modificacion de Código
		 * 07/04/2011 12:49
		 */
