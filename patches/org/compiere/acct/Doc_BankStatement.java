...
public class Doc_BankStatement extends Doc
{
	...
	private DocLine[] loadLines(MBankStatement bs)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			MPayment pay = new MPayment(getCtx(), line.getC_Payment_ID(), getTrxName());
			DocLine_Bank docLine = new DocLine_Bank(line, this);
			//	Changed By Jorge Colmenarez 2014-07-29
			//	Set Date Acct and Conversion Type of the Payment
			//if (i == 0)
			setDateAcct(pay.getDateAcct());
			setC_ConversionType_ID(pay.getC_ConversionType_ID());
			MPeriod period = MPeriod.get(getCtx(), pay.getDateAcct(), line.getAD_Org_ID());
			if (period != null && period.isOpen(DOCTYPE_BankStatement, pay.getDateAcct()))
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if ((!as.isPostIfClearingEqual()) && acct_bank_asset.equals(acct_bank_in_transit)) {
				...
				if (amt_stmt_minus_trx.compareTo(Env.ZERO) != 0) {
					...
					// 	Changed By Jorge Colmenarez 2014-07-29 
					//	call method createLineWithCustomDate and not createLine
					fl = fact.createLineWithCustomDate(line,
					getAccount(Doc.ACCTTYPE_BankAsset, as),line.getC_Currency_ID(), 
					amt_stmt_minus_trx,pay.getDateAcct());
				...
				fl = fact.createLineWithCustomDate(line,
					...
					line.getC_Currency_ID(), line.getStmtAmt(),pay.getDateAcct());
				if (fl != null && AD_Org_ID != 0)
					fl.setAD_Org_ID(AD_Org_ID);
				if (fl != null && C_BPartner_ID != 0)
					fl.setC_BPartner_ID(C_BPartner_ID);
				...
				fl = fact.createLineWithCustomDate(line,
					...
					line.getC_Currency_ID(), line.getTrxAmt().negate(),pay.getDateAcct());
				if (fl != null)
				{
					if (C_BPartner_ID != 0)
						fl.setC_BPartner_ID(C_BPartner_ID);
			...
			if (line.getChargeAmt().compareTo(Env.ZERO) > 0) {
				fl = fact.createLineWithCustomDate(line,
						...
						line.getC_Currency_ID(), null, line.getChargeAmt(),pay.getDateAcct());
			} else {
				// 	Changed By Jorge Colmenarez 2014-07-29 
				//	call method createLineWithCustomDate and not createLine
				fl = fact.createLineWithCustomDate(line,
						line.getChargeAccount(as, line.getChargeAmt().negate()),
						line.getC_Currency_ID(), line.getChargeAmt().negate(), null,pay.getDateAcct());
			}
			if (fl != null && C_BPartner_ID != 0)
				fl.setC_BPartner_ID(C_BPartner_ID);
			...
			if (line.getInterestAmt().signum() < 0) 
				fl = fact.createLineWithCustomDate(line,
					...
					line.getC_Currency_ID(), line.getInterestAmt().negate(),pay.getDateAcct());
			else //	Changed By Jorge Colmenarez 2014-07-29 call method createLineWithCustomDate and not createLine
				fl = fact.createLineWithCustomDate(line,
