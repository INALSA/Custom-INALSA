...
public class RequisitionPOCreate extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (p_M_Requisition_ID != 0)
		{
			...
			//	Validated if C_Project_ID or User1_ID Is Null into Order Header
			if(m_order.getC_Project_ID()==0 && m_order.getUser1_ID()==0){
				if(m_orderLine.getC_Project_ID()!=0)
					m_order.setC_Project_ID(m_orderLine.getC_Project_ID());
				if(m_orderLine.getUser1_ID()!=0)
					m_order.setUser1_ID(m_orderLine.getUser1_ID());
				//	Save Changes
				m_order.saveEx();
			}
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception
	{
		...
		if (m_order == null)
		{
			...
			//	Added By Jorge Colmenarez 2015-06-08
			//	Add Support for C_Project or User1_ID into Header Order
			if(rLine.getParent().get_ValueAsInt("C_Project_ID")!=0)
				m_order.setC_Project_ID(rLine.getParent().get_ValueAsInt("C_Project_ID"));
			if(rLine.getParent().get_ValueAsInt("User1_ID")!=0)
				m_order.setUser1_ID(rLine.getParent().get_ValueAsInt("User1_ID"));
			//	End Jorge Colmenarez
	...
	private void newLine(MRequisitionLine rLine) throws Exception
	{
		...
		//	Added By Jorge Colmenarez 2015-06-08
		//	Add Support for C_Project or User1_ID into Order Line
		if(rLine.get_ValueAsInt("C_Project_ID")!=0)
			m_orderLine.setC_Project_ID(rLine.get_ValueAsInt("C_Project_ID"));
		if(rLine.get_ValueAsInt("User1_ID")!=0)
